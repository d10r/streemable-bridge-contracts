...
contract StreemableERC20Token is ERC20 {
    ...
    function getAccountType() public constant returns(AccountType) {
        return accountTypes[msg.sender];
    }

    function getAccountTypeOf(address acc) public constant returns(AccountType) {
        return accountTypes[acc];
    }

    /** public interface to open a streem */
    function openStreem(address receiver, uint256 flowrate) public returns(uint256) {
        return openStreemImpl(receiver, flowrate, false);
    }

    /** Batch open streems with equal flowrate for deconcentrator accounts */
    function openMultipleStreems(address[] receivers, uint256 flowrate) public {
     ...
     */
    function closeStreem(uint256 id) public {
     ...
     */
    function closeMultipleStreems(uint256[] ids) public {
        require(accountTypes[msg.sender] == AccountType.Deconcentrator);
    ...
    function getStreemBalance(uint256 id) public constant returns(uint256) {
        Streem storage s = streems[id];
    ...
    function hasMinBalance(address acc, uint256 minBal) public constant returns (bool) {
        ...
        for(uint i=0; i< openInStreems.length; i++) {
            tmpBal += int(streemBalance(openInStreems[i]));
            if(tmpBal >= int(minBal)) {
                return true;
            }
        }
    ...
    function openStreemImpl(address receiver, uint256 flowrate, bool batched) internal returns(uint256) {
        ...
        if(accountTypes[msg.sender] == AccountType.OpenAccount) {
            Streem[] memory openOutStreems = getOutStreemsOf(msg.sender);
            require(openOutStreems.length <= OPENACCOUNT_MAX_OUTGOING_STREEMS);
        }
        if(accountTypes[receiver] == AccountType.OpenAccount) {
            Streem[] memory recvOpenInStreems = getInStreemsOf(receiver);
        ...
        if(! batched) {
            ...
            emit StreemOpened(streemId, msg.sender, receiver, flowrate);
        }
    ...
    function exists(Streem s) internal pure returns (bool) {
    ...
    function getOutStreemsOf(address addr) internal constant returns (Streem[]) {
        uint[] storage allArr = outStreemPtrs[addr];
        return filterOpenStreems(allArr);
    }

    function getInStreemsOf(address addr) internal constant returns (Streem[]) {
        uint[] storage allArr = inStreemPtrs[addr];
        return filterOpenStreems(allArr);
    }

    function filterOpenStreems(uint[] inputIds) internal constant returns(Streem[]) {
        // TODO: refactor
    ...
    function equals(Streem s1, Streem s2) internal pure returns (bool) {
    ...
    function expectedStreemBalance(Streem s) internal constant returns (uint256) {
        return (block.timestamp - s.startTime) * s.flowrate;
    }

    function canStreemTo(address sender, address receiver) public constant returns(bool) {
        /* Allowed:
    ...
    function cumulatedExpectedOutStreemsBalanceAndFlowrate(address acc) public constant returns(uint, uint) {
        // TODO: optimize for deconcentrator
        Streem[] memory openOutStreems = getOutStreemsOf(acc);

        uint256 cumOutBalance = 0;
        uint256 cumOutFlowrate = 0;
        for(uint256 i=0; i< openOutStreems.length; i++) {
            Streem memory s = openOutStreems[i];
    ...
    function cumulatedInStreemsBalance(address acc) public constant returns(uint){
        Streem[] memory openInStreems = getInStreemsOf(acc);

        uint256 cumInBalance = 0;
        for(uint i=0; i< openInStreems.length; i++) {
            cumInBalance += streemBalance(openInStreems[i]);
        }
        return cumInBalance;
    }
